...
As mentioned in the YAML header section, `rjournal.csl`, derived from *Biostatistics*, controls the overall referencing style of R Journal papers. For each reference material, you need to provide a link for easy access to the reference and this can be done through either the `doi` or `url` field. If both fields are presented, `url` takes priority and we recommend providing the URL link as `url = {https://doi.org/.../...},`.
