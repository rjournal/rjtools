...
````rmarkdown
```{r demo-html, eval=knitr::is_html_output(), fig.cap = "_Italics_ and a reference to @RJournal in a caption", echo = FALSE}
knitr::include_graphics("Demo.svg")
```
```{r demo-latex, eval=knitr::is_latex_output(), fig.cap = "(ref:demo-caption)", echo = FALSE}
knitr::include_graphics("Demo.pdf")
```
`r if (knitr::is_latex_output()) "(ref:demo-caption) _Italics_ and a reference to @RJournal in a caption"`
