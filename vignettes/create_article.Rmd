...
In the `inst/paper` folder is an example article that has both html and pdf version rendered [manually put in RJournal.sty to render the pdf article] and we will be using it to demonstrate the check functions in the `rjtools`. Notice that this is only an initial automated check and we recommend authors to also read [instructions for Authors](https://journal.r-project.org/share/author-guide.pdf) before submission. 
...
The article title needs to be in the title case and the check is not happy with our title here. Use `toTitleCase()` on the article title  tells us that we forget to capitalise "P" in "package" - better make this change before submission! Also if the title contains the package you're proposing, use the `\pkg{.}` tag rather than `\CRANpkg{.}`.
...
The spell check uses the `hunspell` package with US spelling by default. British spelling can be checked with argument `dic = "en_GB"`. The R journal welcomes both the U.S and British spelling, along with other national variation, as long as consistency is maintained.

The spelling check is performed from the abstract to bibliography section, excluding the comments in the code chunks. Only words with all lowercase letters are checked to avoid picking up on acronyms and names. Notice that the spelling check may pick up some domain specific vocabularies and authors may want to further confirm if the suggested spelling mistakes are true mis-spellings.

In our article, "tooltip" is a specialised vocabulary in interactive graphics, "tsibble" is a package name (we use it to describe the package `tsibbletalk` is used for time series data with a tsibble structure), but "literacture" is a correct pick!
...
and authors need to enter the exact package name for an availability check on CRAN download with the `cranlogs` package.
...
All the packages mentioned with a `\CRANpkg{.}` or `\BIOpkg{.}` tag need to be available on CRAN or Bioconductor and `check_packages_available` makes sure this is the case. 
